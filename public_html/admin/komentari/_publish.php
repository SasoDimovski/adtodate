...
//=========================================================================== 
				...
				$sql = "select id,approved FROM forum WHERE id=".$tmpID;
				$result=mysqli_query($con, $sql);
				$row = mysqli_fetch_array($result);
	     
				if ($row["approved"]==1){$publish=0;}
				if ($row["approved"]==0){$publish=1;}
				//die($sql);
				//print_r ("publish:".($publish)."<br>");  
				mysqli_free_result($result); 

				$sql = "UPDATE forum SET approved =".$publish." WHERE id=".$tmpID;
				$result=mysqli_query($con, $sql);
				//die($sql);  
				//mysqli_free_result($result); 
//=====================================================================================================================================
